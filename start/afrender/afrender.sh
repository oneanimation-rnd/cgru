...
# description: Afanasy afrender daemon
#
### BEGIN INIT INFO
# Provides: afrender
...
# Default-Stop:    0 1 6
# Short-Description: Afanasy afrender daemon
### END INIT INFO
...
# Temp directory:
tmpdir=/Volumes/data/temp/afanasy
if [ ! -d "$tmpdir" ]; then
   mkdir $tmpdir
   if [ ! -d "$tmpdir" ]; then
...
startcmd="$current_dir/_afstart.sh $execfile $logfile"
